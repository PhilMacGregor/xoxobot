...
public class Hra {
    ...
    public boolean jeKonec(int kurzX, int kurzY) {
        Entity konec=null;
        if (volnaPole==0) konec=NIKDO;
        
        int rada=0;
        Entity hrac=sourad[kurzX][kurzY];
            
        for(int i=0;i<sourad.length;i++) {
            if (sourad[kurzX][i]==hrac) rada++;
            else rada=0;
            if (rada>=naKolik) konec=hrac;
        }
        rada=0;
        for(int i=0;i<sourad.length;i++) {
            if (sourad[i][kurzY]==hrac) rada++;
            else rada=0;
            if (rada>=naKolik) konec=hrac;
        }
        rada=0;
        ...
        while ((kurzXX<sourad.length)&&(kurzYY<sourad.length)) {
            if(sourad[kurzXX][kurzYY]==hrac) rada++;
            else rada=0;
            kurzXX++; kurzYY++;
            if (rada>=naKolik) konec=hrac;
        }
        ...
        if(konec!=null) {
            if(konec==NIKDO) JOptionPane.showMessageDialog(null, "Skončilo to remízou!", "Remíza!", INFORMATION_MESSAGE, remiza);
            if(konec==CERVENY) JOptionPane.showMessageDialog(null, "Vyhrál červený!", "Vitězství!", INFORMATION_MESSAGE, cerveny);
            if(konec==MODRY) JOptionPane.showMessageDialog(null, "Vyhrál modrý!", "Vitězství!", INFORMATION_MESSAGE, modry);
            restart();
            return true;
        }
        else return false;   
